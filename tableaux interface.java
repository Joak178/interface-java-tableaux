...
public class TableauIllustrator extends JFrame {
    ...
    /**
     * Prépare l'interface pour l'exécution animée
     */
    private void preparerExecution() {
        ...
        // Réinitialiser l'illustration - rendre tout vide
    ...
    private void executerLigneMethode1() {
        ...
        if (currentLineIndex == 0) {
            // Rendre toutes les cases visibles mais vides (tableau déclaré mais non initialisé)
            for (int i = 0; i < taille; i++) {
            ...
            if (indexTableau < taille) {
                ...
                if (!estValeurValide(valeur, type)) {
                    ...
                    if (illustrationCasePanels != null && indexTableau < illustrationCasePanels.length) {
                        ...
                        illustrationLabels[indexTableau].setText(valeur);
                    }
                }
            }
        }
        ...
        if (currentLineIndex == 0) {
            ...
            if (toutesValides) {
                ...
                for (int i = 0; i < taille; i++) {
                    ...
                    illustrationLabels[i].setText(valeur);
                }
            } else {
    ...
    private boolean estValeurValide(String valeur, String type) {
        ...
        switch (type) {
            ...
            case "char":
                return valeur.length() <= 1;
            case "boolean":
                return valeur.equals("true") || valeur.equals("false");
            case "String":
                return true; // Pas de restriction pour les chaînes
            default:
                return true;
        }
    }
     ...
     * @return Un JPanel contenant le champ et ses délimiteurs
     */
    private JPanel createTypeFieldPanel(String valeurDefaut, String type, int index) {
        ...
        // Détermine les délimiteurs selon le type
        String prefixe = "";
        String suffixe = "";
        ...
        if (!prefixe.isEmpty()) {
            JLabel prefixeLabel = createCodeLabel(prefixe);
            panel.add(prefixeLabel);
        }
        
        // Création et ajout du champ de texte
        JTextField textField = createTypeRestrictedTextField(valeurDefaut, 8, type, index);
        valeursFields[index] = textField;
        panel.add(textField);
        
        // Ajout du suffixe si nécessaire
        if (!suffixe.isEmpty()) {
            JLabel suffixeLabel = createCodeLabel(suffixe);
            panel.add(suffixeLabel);
        }
    ...
    private void updateIllustrationPanel(String type, String nom, int taille, String[] valeurDefauts) {
        ...
        for (int i = 0; i < taille; i++) {
            ...
            // Ajout du préfixe si nécessaire
            if (!prefixe.isEmpty()) {
                JLabel prefixeLabel = new JLabel(prefixe);
                prefixeLabel.setFont(new Font("Monospaced", Font.PLAIN, 14));
                valeurPanel.add(prefixeLabel);
            }
            
            // Création et ajout de l'étiquette pour la valeur
            illustrationLabels[i] = new JLabel("", SwingConstants.CENTER);
            illustrationLabels[i].setFont(new Font("Monospaced", Font.PLAIN, 14));
            valeurPanel.add(illustrationLabels[i]);
            
            // Ajout du suffixe si nécessaire
            if (!suffixe.isEmpty()) {
                JLabel suffixeLabel = new JLabel(suffixe);
                suffixeLabel.setFont(new Font("Monospaced", Font.PLAIN, 14));
                valeurPanel.add(suffixeLabel);
            }
    ...
    private DocumentFilter createFilterForType(String type) {
        return new DocumentFilter() {
            ...
            private final Pattern charPattern = Pattern.compile(".");  // Un seul caractère
    ...
    /**
     * Renvoie une valeur par défaut selon le type
     * @param type Le type de données
    ...
    private String getDefaultValue(String type) {
        switch (type) {
            ...
            case "String": return "";  // Sans guillemets (ajoutés séparément)
            case "char": return "";    // Sans apostrophes (ajoutées séparément)
